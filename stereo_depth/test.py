...
class StereoDepthNode(Node):
    ...
    def process_frames(self):
        # --- Saftey Checks ---
        # Wait until both images and the rectification maps are available
        ...
        # Rectify images (align them so epipolar lines are horizontal)
        rect_l = cv2.remap(gray_l, self.map1_l, self.map2_l, cv2.INTER_LINEAR)
        rect_r = cv2.remap(gray_r, self.map1_r, self.map2_r, cv2.INTER_LINEAR)
        ...
        # This gives a point cloud (X, Y, Z) for each pixel
        point_cloud = cv2.reprojectImageTo3D(disparity_map, self.Q)
        
        # The depth is the Z component of the point cloud
        # We can also mask out invalid depth values (where disparity is 0 or less)
        depth_map = point_cloud[:, :, 2]
        depth_map[disparity_map <= 0] = 0 # Set invalid points to 0 depth

        # --- Visualization ---
        # Normalize disparity and depth for 8-bit display
        disparity_vis = cv2.normalize(disparity_map, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
        
        # For depth, let's clamp the view to a max distance for better visualization
        ...
        if key == ord('q'):
            ...
            # Use a flag or directly shutdown
            raise KeyboardInterrupt # Easiest way to exit the loop and trigger finally block
...
def main(args=None):
    ...
    try:
        ...
        while rclpy.ok():
            rclpy.spin_once(node, timeout_sec=0.01) # Check for new messages
